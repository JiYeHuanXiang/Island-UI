<template>
  ...
  <el-dialog
    ...
    <div>请及时更新海岛到最新版本，这意味着功能增加和 BUG 修复。</div>
    <div>当然，在更新前最好看看右上角的海岛新闻，通常会很有帮助。</div>
    <div>在操作之前，最好能确保你目前可以接触到服务器，以防万一需要人工干预。</div>
    <div>
      <b>如果升级后无法启动，请删除海岛目录中的"update"、"auto_update.exe"并手动进行升级</b>
    </div>
...
const doUpgrade = async () => {
  ...
  ElMessageBox.alert('开始下载更新，请等待……<br>完成后将自动重启海岛，并进入更新流程', '升级', {
    dangerouslyUseHTMLString: true,
  });
  try {
    const ret = await postUpgrade();
    ElMessageBox.alert(ret.text + '<br>如果几分钟后服务没有恢复，检查一下海岛目录', '升级', {
      dangerouslyUseHTMLString: true,
    });
